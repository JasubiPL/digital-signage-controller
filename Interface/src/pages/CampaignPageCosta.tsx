...
export const CampaignPageCosta = () =>{
  ...
  const getCampaign = async () =>{
    const queryData =  await getInfoDb('campanias', 'COSTA')
  ...
  const deleteCampaign = async (name: string) =>{
    const res = await deleteRowsDB("campania", "COSTA", name)
    ...
    <section className="w-full  h-full flex flex-col items-center overflow-y-auto my-8">
      ...
      <section className="w-[90%] flex justify-between mb-4">
        ...
        <img className="h-12" src="/img/brands/logo-costa-azul.webp" alt="Costa Logo" />
        {
          ...
          ? <button 
          onClick={() => setModal(<AddCampaign modal={ setModal } reloadInfo={ getCampaign } company="COSTA"/>)}
          ...
          allCampaign.map( campaign =>(
            <div key={campaign.id} className="text-center grid grid-cols-5 border-b-[1px] border-gray-200 py-2">
              ...
              <div className="flex gap-4 justify-center">
                <button onClick={() => setModal(<CampaignsBoxOffice campaign={campaign.nombre} modal={setModal} company="COSTA" />)} className="hover:scale-110 active:scale-90 transition-all hover:after:content-['Taquillas'] 
                ...
                <button onClick={() => setModal(<EditCampaign modal={setModal} reloadInfo={ getCampaign } data={campaign} company="COSTA"/>)} className="hover:scale-110 active:scale-90 transition-all hover:after:content-['Editar'] 
                after:absolute after:bg-gray-900 after:px-2 after:text-white after:top-[-20px] after:left-0">
